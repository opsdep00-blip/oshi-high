...
### Pixel Data Specification
- Format: `JSON`
- Structure: `{ palette: string[], frames: number[][][] }`
  - `palette`: Array of HEX color codes (max 4 colors).
  - `frames`: 3-dimensional array [frame_index][y_index][x_index].
  - Values in `frames` are indices of the `palette` array.
...
- **ORM**: Prisma
  ...
  - Ensure `Idol` can have multiple `FanTeam` records or a unified `FanTeam` entity.
  - Transactions must be atomic (use `prisma.$transaction`).
...
## Auth.js Implementation Details
- Use `Identity Pooling`: A single `User` record can have multiple `Accounts` (SMS for Fans, SNS for Idols).
- For "Idol Claiming", the SNS profile ID must match the `sns_link` defined in the `Idol` entity.

### Anti-Fraud & Constraints
- One phone number (SMS) can support only **ONE** Idol at a time.
- Switching targets may have a cooldown period (to be implemented).
- Support transactions must verify the `phone_hash` in the database before incrementing points.
