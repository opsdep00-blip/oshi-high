...
3. 実装・修正: 小さな単位で実装（関数単位 / コンポーネント単位）。
4. 自動検査: TypeScript compile、ESLint、ユニットテスト実行。
5. PR 作成: Conventional Commits + コード品質チェックリスト含む。
6. レビュー・フィードバック対応: CI グリーン → マージ。
...
*このファイルは Codex の公式定義（Plan形式）です。コード品質・セキュリティ管理の専任エージェントとして機能します。*