...
output "database_url" {
  value       = "postgresql://${google_sql_user.oshi_user.name}:${random_password.db_password.result}@/${google_sql_database.oshi_high_staging.name}?host=/cloudsql/${google_sql_database_instance.staging.connection_name}"
  sensitive   = true
  description = "Full DATABASE_URL for Cloud Run (store as GitHub Secret STAGING_DATABASE_URL)"
}
