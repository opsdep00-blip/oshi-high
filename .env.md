...
|----------|---------|-------|------------|
| `ENABLE_SMS_MOCK` | Mock SMS sending (Firebase mock) | `"true"` | Always true in `.env.local` |
| `SMS_PROVIDER` | Provider name when mock is false | `firebase` | For staging/prod; repository uses Firebase by default |
...
|----------|---------|------------|-----------|
| `SMS_PROVIDER` | Provider name when mock is false | Secret Manager | `firebase` (default) |
| `FIREBASE_PROJECT_ID` | GCP Firebase Project ID | GCP Secret Manager | Used for SMS (Firebase) and other Firebase APIs |
| `FIREBASE_CLIENT_EMAIL` | GCP Service Account Email | GCP Secret Manager | For Firebase Admin / service account usage |
| `FIREBASE_PRIVATE_KEY` | GCP Service Account Private Key | GCP Secret Manager | For authenticated Firebase API calls |
...
if (process.env.ENABLE_SMS_MOCK === "true") {
  ...
  // Use real SMS provider (Firebase)
}
